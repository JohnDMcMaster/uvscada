...
640x480|1024x768|1280x960|1600x1200(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0020, 0x0344, buff, 1, 500);
3264x2448(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0008, 0x0344, buff, 1, 500);)
if (validate_read((char[]){0x08}, 1, buff, n_rw, "packet 19/20") < 0)
	return 1;
//Generated from packet 21/22
640x480|1280x960|1600x1200(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0CA1, 0x0348, buff, 1, 500);)
1024x768(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0C21, 0x0348, buff, 1, 500);)
3264x2448(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0CC9, 0x0348, buff, 1, 500);)
if (validate_read((char[]){0x08}, 1, buff, n_rw, "packet 21/22") < 0)
	return 1;
//Generated from packet 23/24
640x480|1024x768|1280x960(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0020, 0x0346, buff, 1, 500);)
1600x1200(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0008, 0x0346, buff, 1, 500);)
3264x2448(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0000, 0x0346, buff, 1, 500);)
if (validate_read((char[]){0x08}, 1, buff, n_rw, "packet 23/24") < 0)
	return 1;
//Generated from packet 25/26
640x480|1280x960(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0981, 0x034A, buff, 1, 500);)
1024x768(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0921, 0x034A, buff, 1, 500);)
1600x1200(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0969, 0x034A, buff, 1, 500);)
3264x2448(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0991, 0x034A, buff, 1, 500);)
if (validate_read((char[]){0x08}, 1, buff, n_rw, "packet 25/26") < 0)
	return 1;
//Generated from packet 27/28
640x480|1024x768(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x02FC, 0x3040, buff, 1, 500);)
1280x960|1600x1200(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x026C, 0x3040, buff, 1, 500));
3264x2448(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0224, 0x3040, buff, 1, 500);)
if (validate_read((char[]){0x08}, 1, buff, n_rw, "packet 27/28") < 0)
	return 1;
//Generated from packet 29/30
640x480|1024x768|1280x960(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0002, 0x0400, buff, 1, 500);)
1600x1200|3264x2448(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0000, 0x0400, buff, 1, 500);)
if (validate_read((char[]){0x08}, 1, buff, n_rw, "packet 29/30") < 0)
	return 1;
//Generated from packet 31/32
640x480|1280x960(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0014, 0x0404, buff, 1, 500);)
1024x768(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0018, 0x0404, buff, 1, 500);)
1600x1200|3264x2448(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0010, 0x0404, buff, 1, 500);)
if (validate_read((char[]){0x08}, 1, buff, n_rw, "packet 31/32") < 0)
	return 1;
//Generated from packet 33/34
//0x034C: width
640x480(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0280, 0x034C, buff, 1, 500);)
1024x768(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0400, 0x034C, buff, 1, 500);)
1280x960(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0500, 0x034C, buff, 1, 500);)
1600x1200(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0640, 0x034C, buff, 1, 500);)
3264x2448(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0CC0, 0x034C, buff, 1, 500);)
if (validate_read((char[]){0x08}, 1, buff, n_rw, "packet 33/34") < 0)
	return 1;
//Generated from packet 35/36
//0x034E: height
640x480(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x01E0, 0x034E, buff, 1, 500);)
1024x768(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0300, 0x034E, buff, 1, 500);)
1280x960(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x03C0, 0x034E, buff, 1, 500);)
1600x1200(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x04B0, 0x034E, buff, 1, 500);)
3264x2448(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0990, 0x034E, buff, 1, 500);)
if (validate_read((char[]){0x08}, 1, buff, n_rw, "packet 35/36") < 0)
	return 1;
//Generated from packet 37/38
640x480(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x02C0, 0x300A, buff, 1, 500);)
1024x768(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x033C, 0x300A, buff, 1, 500);)
1280x960(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0400, 0x300A, buff, 1, 500);)
1600x1200(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x04F0, 0x300A, buff, 1, 500);)
3264x2448(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0A00, 0x300A, buff, 1, 500);)
if (validate_read((char[]){0x08}, 1, buff, n_rw, "packet 37/38") < 0)
	return 1;
//Generated from packet 39/40
640x480|1024x768|1280x960|1600x1200(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x0E00, 0x300C, buff, 1, 500);)
3264x2448(n_rw = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0x0B, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE, 0x2600, 0x300C, buff, 1, 500);)
...
640x480|1024x768|1280x960|1600x1200(if (validate_read((char[]){0x0E, 0x00, 0x08}, 3, buff, n_rw, "packet 65/66") < 0))
3264x2448(if (validate_read((char[]){0x26, 0x00, 0x08}, 3, buff, n_rw, "packet 65/66") < 0))
	return 1;
