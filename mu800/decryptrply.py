...
def printControlRequest(submit, data_str, data_size, pipe_str,
                        print_bRequestType, print_bRequest, print_wValue, print_wIndex):
    '''
    ...
    if args.ofmt == OUTPUT_LIBUSBPY:
        ...
        if (print_bRequestType & URB_TRANSFER_IN):
            print "%sbuff = dev.controlRead(0x%02X, 0x%02X, 0x%04X, 0x%04X, %u)" % (indent, print_bRequestType, print_bRequest,
                    print_wValue, print_wIndex, data_size)
        else:
            print "%sdev.controlWrite(0x%02X, 0x%02X, 0x%04X, 0x%04X, %s)" % (indent, print_bRequestType, print_bRequest,
                    print_wValue, print_wIndex, data_str)
    elif args.ofmt == 'JSON':
        # FIXME
        pass
    else:
        ...
        bRequestStr = request_type2str[ self.print_bRequestType, self.print_bRequest ]
        bRequestTypeStr = ""
        
        if args.ofmt == OUTPUT_LIBUSB and not g_use_defines:
            bRequestTypeStr = "0x%02X" % self.print_bRequestType
        else:
            bRequestTypeStr = request_type2str[self.print_bRequestType]
...
class Gen:
    ...
    def processControlCompleteIn(self, dat_cur):
        ...
        print_bRequestType = self.submit.m_ctrl.bRequestType
        print_bRequest = self.submit.m_ctrl.bRequest
        print_wValue = self.submit.m_ctrl.wValue
        print_wIndex = self.submit.m_ctrl.wIndex
        
        # look for key packet
        # buff = dev.controlRead(0xC0, 0x16, 0x171F, 0x0000, 2)
        # bmRequestType: 0xC0
        # bRequest: 0x16
        if self.submit.m_ctrl.bRequestType == 0xC0 and self.submit.m_ctrl.bRequest == 0x16:
            print '# Found the key!'
            print_wValue = 0
            check = rright16(self.submit.m_ctrl.wValue, 4)
        # is it a packet that we need to rekey?
        elif self.submit.m_ctrl.bRequestType == 0xC0 and self.submit.m_ctrl.bRequest == 0x0B:
            print_wValue = decrypt16(self.submit.m_ctrl.wValue)
            print_wIndex = decrypt16(self.submit.m_ctrl.wIndex)
    ...
    def processControlCompleteOut(self, dat_cur):
        ...
        printControlRequest(self.submit, data_str, data_size, "usb_sndctrlpipe(%s, 0), " % (deviceStr()),
                self.submit.m_ctrl.bRequestType, self.submit.m_ctrl.bRequest, self.submit.m_ctrl.wValue, self.submit.m_ctrl.wIndex)
