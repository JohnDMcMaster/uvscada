...
#ifdef GSPCA_DEBUG
int gspca_debug = D_ERR | D_PROBE | 0xFF;
					  ...
					  int xfer)
{
	...
	for (i = 0; i < alt->desc.bNumEndpoints; i++) {
		...
		info("attr raw 0x%02X & 0x%02X => 0x%02X", ep->desc.bmAttributes, USB_ENDPOINT_XFERTYPE_MASK, attr);
		if (attr == xfer
...
static struct usb_host_endpoint *get_ep(struct gspca_dev *gspca_dev)
{
	...
	printk(KERN_ALERT "Getting endpoint\n");
	intf = usb_ifnum_to_if(gspca_dev->dev, gspca_dev->iface);
	ep = NULL;
	xfer = gspca_dev->cam.bulk ? USB_ENDPOINT_XFER_BULK
				   : USB_ENDPOINT_XFER_ISOC;
	info("bulk: %d, xfer 0x%02X", gspca_dev->cam.bulk, xfer);
	i = gspca_dev->alt;			/* previous alt setting */
	if (gspca_dev->cam.reverse_alts) {
		info("reverse alts yep, i: %d, nbalt: %d", i, gspca_dev->nbalt);
			...
			struct usb_host_endpoint *ep)
{
	...
	if (!gspca_dev->cam.bulk) {		/* isoc */
		printk(KERN_ALERT "gspca dev 0x%p: creating urbs for isoc\n", gspca_dev);
		...
		printk(KERN_ALERT "gspca dev 0x%p: creating urbs for bulk\n", gspca_dev);
		npkt = 0;
...
static int gspca_init_transfer(struct gspca_dev *gspca_dev)
{
	...
	for (;;) {
		...
		if (gspca_dev->cam.bulk)
			if (gspca_dev->urb[0] == NULL) {
				printk(KERN_ALERT "Almost oops, gspca_dev->urb[0] NULL\n");
				ret = -EIO;
				goto out;
			}
...
{
	info("uvmain v%d.%d.%d registered",
