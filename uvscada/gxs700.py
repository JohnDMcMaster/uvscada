...
class GXS700:
    ...
    ***************************************************************************
    ...
    def _init(self):
        ...
        {1: self._setup_fpga1_sm,
         2: self._setup_fpga1_lg}[self.size]()

        v = self.fpga_rsig()
        if v != 0x1234:
            raise Exception("Invalid FPGA signature: 0x%04X" % v)

        {1: self._setup_fpga2_sm,
         2: self._setup_fpga2_lg}[self.size]()
