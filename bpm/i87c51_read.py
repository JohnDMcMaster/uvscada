...
def fw_read(dev, cont, target=4096):
    ...
    if cont:
        ...
        #buff = bulk2(dev, "\x08\x00\x57\x8C\x00", target=len(fw))
        buff = bulk2(dev, "\x08\x00\x57\x8C\x00", target=target)
    return buff
...
def replay1(dev, cont=True):
    ...
    buff = bulk2(dev, fwm.p201, target=0x02)
    # Discarded 510 / 512 bytes => 2 bytes
    ...
    if cont:
        ...
        buff = bulk2(dev, fwm.p221, target=0x02)
        # Discarded 510 / 512 bytes => 2 bytes
        ...
        buff = bulk2(dev, fwm.p233, target=0x02)
        # Discarded 510 / 512 bytes => 2 bytes
    ...
    if cont:
        ...
        buff = bulk2(dev, fwm.p257, target=0x02)
        # Discarded 510 / 512 bytes => 2 bytes
    ...
    buff = bulk2(dev, fwm.p281, target=0x02)
    if cont:
        # Discarded 510 / 512 bytes => 2 bytes
    ...
    buff = bulk2(dev, fwm.p295, target=0x02)
    if cont:
        # Discarded 510 / 512 bytes => 2 bytes
    ...
    buff = bulk2(dev, fwm.p323, target=0x02)
    if cont:
        # Discarded 510 / 512 bytes => 2 bytes
    ...
    buff = bulk2(dev, fwm.p351, target=0x02)
    if cont:
...
def replay2(dev, cont):
    ...
    buff = bulk2(dev, fwm.p373, target=0x02)
    if cont:
        # Discarded 510 / 512 bytes => 2 bytes
